...
if(isset($_POST['submit']))
			{
				...
				$polaczenie = new mysqli("localhost","root","","waluty");
					$sql = "Select * from spolki where walor = '$nazwa' order by id_spolki desc";
					$wynik = $polaczenie->query($sql);
					$ile = $wynik->num_rows;
					if($ile ==  0)
	{
		header("Location: znajdz_akcje.php?id=0");
		exit;
	}
	else
	{		
		...
		$id = $row['id_spolki'];
			header("Location: znajdz_akcje.php?id=$id");
			exit;
	}
			}
...
<select name = "nazwa">
  ...
  <option value="PZU">Powszechny Zakład Ubezpieczen SA</option>
</select>
<br>
